...
def get_row_echelon_form(matrix):
    ...
    for i in range(m):
        ...
        if cur_row != non_zero_row:
            print(f"Переставим местами строки {cur_row + 1} и {non_zero_row + 1}")
        ...
        print(f"Поделим строку {cur_row + 1} на {matrix[cur_row][i]}")
        ...
        for j in range(cur_row + 1, n):
            ...
            print(f"Из строки {j+1} вычтем строку {cur_row + 1}, умноженную на {matrix[j][i]}")
